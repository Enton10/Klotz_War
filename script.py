...
font_mid = pygame.font.SysFont("arial", 42, bold=True)
...
class Snake:
    def __init__(self, name, start_pos, color, controls, size=20):
        ...
        self.size = size
        self.controls = controls  # dict: up,down,left,right
        self.rect = pygame.Rect(start_pos[0], start_pos[1], size, size)
        self.speed = 5
        self.trail = []          # Liste von (x, y, size)
        self.max_trail = 18
        self.alive = True
        self.dying = False
        self.death_timer = 0     # Frames für Sterbe-Animation
        self.death_max = 50
    ...
    def draw(self, surface):
        ...
        # Body
        if self.dying:
            # Sterbe-Animation: pulsiert rot, expandiert und verblasst
...
def menu():
    ...
    subtitle = font_small.render(
        "Wähle die Anzahl der Spieler", True, (200, 200, 200))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if btn2.clicked(event.pos):
                    return 2
                if btn3.clicked(event.pos):
                    return 3
                if btn_quit.clicked(event.pos):
                    return None
...
class Wechsler:
    ...
    def draw(self, surface):
        if self.active:
            ...
            pygame.draw.circle(surface, (0, 230, 230),
                               self.rect.center, r)
            pygame.draw.circle(surface, (255, 255, 255),
...
def play_game(num_players):
    ...
    while running:
        ...
        # Kollisionen zwischen lebenden Schlangen
        ...
        for i in range(len(alive_snakes)):
            for j in range(i + 1, len(alive_snakes)):
                ...
                if a.collides_with(b):
                    # zufällig stirbt einer; bei Größenunterschied gewinnt
                    # der Größere mit höherer Wahrscheinlichkeit
                    total = a.size + b.size
